...
# "Aventureiro": compilador C, make, pkg-config, headers de ncurses, e
# python3/pip (só para `make test`, que usa pexpect). Não instala nada
...
if [[ -z "$SO" ]]; then
    ...
    echo "Instale manualmente: compilador C, make, pkg-config, headers de ncurses, python3 e pip."
    exit 1
fi
...
if [[ ${#FALTANDO[@]} -eq 0 ]]; then
    ...
    case "$SO-$GERENCIADOR" in
        macos-brew)
            ...
            COMANDO_INSTALL="brew install pkg-config ncurses"
            ;;
        linux-apt)
            COMANDO_INSTALL="sudo apt-get update && sudo apt-get install -y build-essential pkg-config libncurses-dev python3 python3-pip"
            ;;
        linux-dnf)
            COMANDO_INSTALL="sudo dnf install -y gcc make pkgconf-pkg-config ncurses-devel python3 python3-pip"
            ;;
        linux-pacman)
            COMANDO_INSTALL="sudo pacman -S --needed base-devel pkgconf ncurses python python-pip"
            ;;
        *)
...
# --- Dependencia opcional: pexpect (so' para 'make test') ------------------

echo
if tem_comando python3 && python3 -c "import pexpect" >/dev/null 2>&1; then
    verde "pexpect ja instalado (make test pronto pra rodar)."
else
    amarelo "pexpect nao encontrado - so' e' necessario para 'make test' (fuzzing automatizado), nao para jogar."
    echo "Comando sugerido:"
    ...
    if confirmar "Rodar esse comando agora?"; then
        ...
        echo "Ok, nada instalado. Rode o comando acima manualmente se for usar 'make test'."
    fi
fi
